		...
		<Tooltip>
			<TooltipTrigger asChild>
				<Button
					...
					size={label ? "sm" : "icon"}
					className={
						label ? "h-8 flex flex-col gap-0 px-2 min-w-10" : "h-8 w-8"
					}
					onClick={onClick}
				>
			...
			<Tabs defaultValue="home" className="w-full">
				<div className="flex items-center bg-muted/30 px-4 pt-1">
					<TabsList className="bg-transparent h-8 space-x-1 p-0">
						<TabsTrigger
							value="file"
							className="px-3 h-7 data-[state=active]:bg-background data-[state=active]:border-b-0 rounded-t-sm rounded-b-none text-xs"
						>
							File
						</TabsTrigger>
						<TabsTrigger
						...
						<TabsTrigger
							value="insert"
							className="px-4 h-7 data-[state=active]:bg-background data-[state=active]:border-b-0 rounded-t-sm rounded-b-none text-xs"
						>
							Insert
						</TabsTrigger>
						<TabsTrigger
				...
				<div className="h-20 bg-background px-4 py-1 flex overflow-x-auto">
					<TabsContent
						value="file"
						className="m-0 h-full flex items-center gap-2"
					>
						...
						<RibbonGroup label="Cells">
							<div className="grid grid-cols-2 gap-1">
								<RibbonButton
									icon={PlusSquare}
									label="Insert"
									tooltip="Insert row/column"
									onClick={onInsertRow}
								/>
								<RibbonButton
									icon={MinusSquare}
									label="Delete"
									tooltip="Delete row/column"
									onClick={onDeleteRow}
								/>
							</div>
						</RibbonGroup>
						...
						<RibbonGroup label="Illustrations">
							...
							<div className="flex gap-1">
								<RibbonButton
									icon={PlusSquare}
									label="Shapes"
									tooltip="Insert Shapes"
									onClick={() => onInsertShape?.("rectangle")}
								/>
								<RibbonButton
									icon={Type}
									label="Icons"
									tooltip="Insert Icons"
					...
					>
						<RibbonGroup label="Function Library">
							<RibbonButton
								icon={Sigma}
								label="AutoSum"
								tooltip="Sum of selected cells"
								onClick={() => onInsertFunction?.("SUM")}
							/>
							<RibbonButton
								icon={Clock}
								label="Recently Used"
								tooltip="Recent formulas"
								onClick={() => {}}
							/>
						</RibbonGroup>
						...
						<RibbonGroup label="Logical">
							<RibbonButton
								icon={Activity}
								label="Logical"
								tooltip="AND, OR, NOT, IF"
								onClick={() => onInsertFunction?.("IF")}
							/>
						</RibbonGroup>
						<RibbonGroup label="Text">
							<RibbonButton
								icon={Type}
								label="Text"
								tooltip="LEN, CONCAT, UPPER"
								onClick={() => onInsertFunction?.("LEN")}
							/>
						</RibbonGroup>
						<RibbonGroup label="Statistical">
							<RibbonButton
								icon={BarChart}
								label="Statistical"
								tooltip="STDEV, VAR, MEDIAN"
								onClick={() => onInsertFunction?.("AVERAGE")}
							/>
						</RibbonGroup>
					</TabsContent>

...
const isPatternFill = (
	fill: ExcelJS.Fill,
): fill is ExcelJS.Fill & ExcelPatternFill => {
...
export const useExcelService = () => {
				...
				rowIndices.forEach((rowIndex) => {
					...
					colIndices.forEach((colIndex) => {
						...
						if (cellData.formula?.startsWith("=")) {
							cell.value = {
								formula: cellData.formula.substring(1),
	...
	const addExcelFeatures = useCallback(
		(workbook: ExcelJS.Workbook): ExcelJS.Workbook => {
			const worksheet = workbook.worksheets[0];

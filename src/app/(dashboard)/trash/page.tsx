    ...
    return (
        <div className="container mx-auto py-10 px-6 font-sans">
            <div className="mb-8">
                ...
                <p className="text-muted-foreground mt-1">Restore or permanently delete your spreadsheets.</p>
            </div>
                    ...
                    {spreadsheets.map((sheet) => (
                        <Card key={sheet.id} className="group overflow-hidden">
                            ...
                            <CardHeader className="p-4">
                                <CardTitle className="truncate" title={sheet.name}>{sheet.name}</CardTitle>
                                <CardDescription className="text-xs mt-1">
                                    Deleted: {new Date(sheet.deletedAt!).toLocaleDateString()}
                                </CardDescription>
                            </CardHeader>
                            <CardContent className="p-4 pt-0 flex gap-2 justify-end">
                                <Button variant="outline" size="sm" onClick={() => restoreSpreadsheet(sheet.id)} title="Restore">
                                    <RotateCcw className="h-4 w-4 mr-1" />
                                    Restore
                                </Button>
                                <Button variant="destructive" size="sm" onClick={() => permanentlyDelete(sheet.id)} title="Delete Permanently">

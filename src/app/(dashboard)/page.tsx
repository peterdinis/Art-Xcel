      ...
      <main className="container mx-auto py-10 px-6">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-muted-foreground mt-1">Manage your spreadsheets and create new ones.</p>
          </div>
          <Button onClick={createNewSpreadsheet} size="lg" className="gap-2 shadow-lg hover:shadow-xl transition-all">
        ...
        {spreadsheets.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {spreadsheets.map((sheet) => (
              <Link href={`/editor/${sheet.id}`} key={sheet.id} className="group">
                <Card className="h-full transition-all duration-200 hover:shadow-lg hover:border-primary/50 cursor-pointer overflow-hidden group-hover:-translate-y-1">
                  ...
                  <CardHeader className="p-4">
                    <CardTitle className="truncate pr-4" title={sheet.name}>{sheet.name}</CardTitle>
                  ...
                  <CardContent className="flex justify-end p-2">
                    <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive hover:text-destructive hover:bg-destructive/10" onClick={(e) => deleteSpreadsheet(e, sheet.id)}>

...
export default function SpreadsheetError({ error, reset }: SpreadsheetErrorProps) {
  useEffect(() => {
      ...
      >
        ...
        >
          <h2 className="text-2xl font-semibold text-foreground">
            ...
            {!isNetworkError && !isPermissionError && !isNotFoundError && "Spreadsheet Error"}
          </h2>
          
          <div className="bg-muted p-4 rounded-lg">
            <p className="text-muted-foreground">
              {isNetworkError && "Unable to connect to the server. Please check your internet connection."}
              {isPermissionError && "You don't have permission to access this spreadsheet."}
              {isNotFoundError && "The spreadsheet you're trying to access doesn't exist."}
              {!isNetworkError && !isPermissionError && !isNotFoundError && error.message}
            </p>
          </div>

          {/* Recovery suggestions */}
          <div className="text-sm text-muted-foreground">
            {isNetworkError && "Try refreshing the page or check your connection"}
        ...
        >
          ...
          <Button asChild variant={isPermissionError || isNotFoundError ? "default" : "outline"} className="gap-2">
            <Link href="/">
